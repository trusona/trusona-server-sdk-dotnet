...
namespace TrusonaSDK.HTTP.Client.Security
{
  public class RequestHmacMessage : IHmacMessage
  {
    ...
    private static string GetContentType(HttpContent content) => content?.Headers.ContentType.ToString();

    private static string GetHeaderValue(HttpRequestHeaders headers, string name) => headers.TryGetValues(name, out IEnumerable<string> headerValues) ? headerValues.First() : null;
